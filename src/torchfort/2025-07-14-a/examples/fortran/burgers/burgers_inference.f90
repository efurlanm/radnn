program burgers_inference
  ...
  allocate(u_pred_reshaped(N_t, N_x))
  ...
  ! Populate XT_tensor (features, total_points)
  do j = 1, N_t
    do i = 1, N_x
  ...
  !--------------------------------------------------------------------
  istat = torchfort_create_model("burgers_inference_model", "config_burgers_inference.yaml", device)
  ...
  istat = torchfort_load_model("burgers_inference_model", "burgers_model_trained.pt")
  if (istat /= TORCHFORT_RESULT_SUCCESS) then
  ...
  istat = torchfort_inference_multiarg("burgers_inference_model", input_tensors, output_tensors)
  if (istat /= TORCHFORT_RESULT_SUCCESS) then
  ...
  do j = 1, N_t
    do i = 1, N_x
      u_pred_reshaped(j, i) = u_pred_flat(1, (j-1)*N_x + i)
    end do
  end do
  ...
  !--------------------------------------------------------------------
  open(unit=10, file="examples/fortran/burgers/burgers1d_fortran_inference_results.bin", &
