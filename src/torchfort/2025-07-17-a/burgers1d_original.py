...
N_0 = 400    # Initial condition points (Updated from 200)
...
class PINN(nn.Module):
    def __init__(self, layers):
        ...
        # Build the network using a list of layers
        net_layers = []
...
for epoch in range(num_epochs):
    ...
    # Print loss every 5000 epochs to track progress (updated from 500)
    if (epoch+1) % 500 == 0:
...
# Save the directly predicted solution from the original script
output_direct_filename = "original_python_u_pred_direct.txt"
np.savetxt(output_direct_filename, u_pred)
print(f"Direct Python u_pred saved to {output_direct_filename}")

# Save the model state dictionary
state_dict_filename = "pinn_model_state.pt"
torch.save(model.state_dict(), state_dict_filename)
print(f"Model state dictionary saved to {state_dict_filename}")

# Create a new model instance and load the state dictionary, then script it
new_model_for_scripting = PINN(layers).to(device)
new_model_for_scripting.load_state_dict(torch.load(state_dict_filename))
new_model_for_scripting.eval()
scripted_model_filename = "burgers_inference_model.pt"
torch.jit.script(new_model_for_scripting).save(scripted_model_filename)
print(f"Trained PINN model (reloaded and scripted) saved to {scripted_model_filename}")
