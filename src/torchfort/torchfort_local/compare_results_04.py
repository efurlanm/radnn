...
def compare_results_04():
    ...
    fortran_trained_model_path = "./examples/fortran/burgers/burgers_model_trained_04.pt"
    
    try:
        # Load the scripted BurgersPINN model (which contains the .net submodule)
        loaded_model = torch.jit.load(fortran_trained_model_path, map_location=torch.device("cpu")) # Force CPU for loading
        ...
        x_plot_inf = np.linspace(x_min, x_max, num_points_x)
        t_plot_inf = np.linspace(t_min, t_max, num_points_t)

        X_grid_inf, T_grid_inf = np.meshgrid(x_plot_inf, t_plot_inf)
        
        t_flat_inf = torch.tensor(T_grid_inf.flatten(), dtype=torch.float32).unsqueeze(1).to(torch.device("cpu"))
        x_flat_inf = torch.tensor(X_grid_inf.flatten(), dtype=torch.float32).unsqueeze(1).to(torch.device("cpu"))

        # Perform inference using the .net submodule of the loaded BurgersPINN
        with torch.no_grad():
            u_inference_fortran_trained_flat = loaded_model.net(torch.cat([t_flat_inf, x_flat_inf], dim=1)).cpu().numpy()

        U_fortran_trained = u_inference_fortran_trained_flat.reshape(num_points_t, num_points_x)
        ...
        print("Please ensure burgers04.f90 has been run successfully.")
        return
    ...
    plt.pcolormesh(x_plot_orig, t_plot_orig, abs_diff, cmap='hot', shading='auto')
