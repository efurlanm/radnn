program burgers
  ...
  real(real32) :: nu
  integer, parameter :: N_f = 10000, N_0 = 200, N_b = 200
  real(real32), parameter :: x_min = -1.0, x_max = 1.0, t_min = 0.0, t_max = 1.0
  integer, parameter :: num_epochs = 10000 ! Number of training epochs

  type(torchfort_tensor_list) :: inputs_list, labels_list
  real(real32) :: loss_val
  real(real32), allocatable :: dummy_label_tensor(:) ! Changed to allocatable array
  ...
  if (istat /= 0) then
     print *, "Error destroying inputs_list"
     stop
  endif
  istat = torchfort_tensor_list_destroy(labels_list)
  if (istat /= 0) then
     print *, "Error destroying labels_list"
     stop
  endif
