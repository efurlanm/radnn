...
<!-- -->
    ...
    Markdown
    : Uma linguagem de marcação leve com sintaxe de formatação de texto simples.
    É frequentemente usado para formatar arquivos readme, escrever mensagens em fóruns online e criar texto rico usando um editor de texto comum.

    HTML
    : HyperText Markup Language (Linguagem de Marcação de Hipertexto) é a linguagem de marcação padrão para documentos projetados para serem exibidos em um navegador da web.
    Pode ser auxiliado por tecnologias como Cascading Style Sheets (CSS) e linguagens de script como JavaScript.
...
<!-- -->
    ...
    `git commit`
    : Este comando registra as alterações no repositório. Use com a flag `-m` para adicionar uma mensagem: `git commit -m "Mensagem do commit"`.

    *Itálico*
    : Texto com ênfase em itálico.

    **Negrito**
    : Texto com ênfase em negrito.
